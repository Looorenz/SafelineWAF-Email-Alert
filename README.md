...
```bash
# Verify SafeLine is running
docker ps | grep safeline

# Test database connection
docker exec safeline-pg psql -U safeline-ce -d safeline-ce -c "SELECT 1"
```
...
```bash
# Check permissions
ls -la /opt/safeline-monitoring/bin/

# Reapply permissions
sudo chmod 750 /opt/safeline-monitoring/bin/*.sh
```
...
```bash
# Monitor log
tail -n 50 /opt/safeline-monitoring/logs/monitor.log

# Mailer log
tail -n 50 /opt/safeline-monitoring/logs/mailer.log

# Recorded attacks
tail -n 5 /opt/safeline-monitoring/logs/attacks.jsonl | jq '.'
```

## Useful Queries

Analyze attack logs with jq:

```bash
# Count attacks by type
jq -r '.attack_name' /opt/safeline-monitoring/logs/attacks.jsonl | sort | uniq -c

# Critical attacks only
jq 'select(.risk_level == 3)' /opt/safeline-monitoring/logs/attacks.jsonl

# Top 10 source IPs
jq -r '.src_ip' /opt/safeline-monitoring/logs/attacks.jsonl | sort | uniq -c | sort -rn | head -10

# Attacks by host
jq -r '.host' /opt/safeline-monitoring/logs/attacks.jsonl | sort | uniq -c

# Last 5 recorded attacks
tail -5 /opt/safeline-monitoring/logs/attacks.jsonl | jq '.'
```
